...
class PETC_Generation_NL():
    def __init__(self, n_traces, tmax, delta_t, sigma_tr, seed):
    ...
    def compute_trajectories_dist(self):
        ...
        for idx, init_y in enumerate(init_conditions):
            ...
            if len(t_sampled) > 1:
                ...
                warnings.warn('Error Encountered in LSODA integration')
    ...
    def event_triggered_discrete_ist(self, xi):
        ...
        def error_dynamics(t, y, ):
            ...
            # e' = -f(x, u(x_t_k))
            x_t_k = y[0:2] + y[2:4]
            ctrl = [0, - x_t_k[0] * x_t_k[1] ** 2 - x_t_k[1] ** 3]
            fx = np.array([- y[0] ** 3 + y[0] * y[1] ** 2 + ctrl[0], y[0] * y[1] ** 2 - y[0] ** 2 * y[1] + ctrl[1]])
            dydt = np.concatenate([fx, -fx])
            return dydt
    ...
    def get_input_output(self, desired_components, stdize):
        ...
        if len(desired_components) == 1 and desired_components[0] != 'polar':
        ...
        if stdize:
            ...
            print('Info: Data are *not* standardized')
        ...
        if 'polar_min_one' in desired_components:
            # current polars
            pc = ns.convert_spherical(self.single_xi_sampled)
            for idx, c in enumerate(pc.T):
                if idx > 0:
                    pc[:, idx] = c % np.pi * np.prod(np.sign(self.single_xi_sampled), axis=1)
                components_x.append(c)
            # polars at k-1
            pc_min_one = ns.convert_spherical(self.single_xi_min_one)
            for idx, c in enumerate(pc_min_one.T):
                if idx > 0:
                    pc_min_one[:, idx] = c % np.pi * np.prod(np.sign(self.single_xi_min_one), axis=1)
                components_x.append(c)
            # compute mean and std of current pc
            for i in range(pc.shape[1]):
                mean = (pc[:, i]).mean()
                std = (pc[:, i]).std()
                std_pc_min_one = (pc_min_one[:, i] - mean) / std
                x = np.hstack([x, std_pc_min_one[:, None]])
    ...
    def get_data(self, desired_components):
        """
        ...
        x, y = self.get_input_output(desired_components, stdize=False)
