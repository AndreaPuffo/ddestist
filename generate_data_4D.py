...
data_generator = PETC4D(n_traces=N_traces, tmax=tmax, delta_t=delta_t,
...
data_df = pd.DataFrame(data=np.array([X[0,:], X[1,:], X[2,:], X[3,:], Y]).T, columns=['norm_xi', 'angles', 'angles', 'angles', 'ist'])
