...
"""
from 
Computing the average inter-sample time of event-triggered control using
...
class PETC3D():
    ...
    def event_triggered_discrete_ist(self, xi):
        ...
        def event(t, xi):
            # sigma * x ** 2 - err ** 2 = 0
            return sum(xi[3:] ** 2) - self.sigma_trig * sum(xi[0:3] ** 2)

        event.terminal = True
        # event.direction = 0

        def plant_dynamics(K, y):
            ...
            A_tot = np.block([[Ap + Bp @ K, Bp @ K], [-Ap - Bp @ K, -Bp @ K]])
            dydt = A_tot @ y
        ...
        stop = False
        while not stop:
            sol = solve_ivp(error_dynamics, (tstart, self.tmax), xi, args=(-self.K,),
            ...
            for idx, y in enumerate(ys[-1].T):
                ...
                if idx == ys[-1].shape[1] - 1:
                    stop = True
                # otherwise, check
                if sum(y[3:] ** 2) - self.sigma_trig * sum(y[0:3] ** 2) > 0.0:
                    tstart = ts[-1][idx]
